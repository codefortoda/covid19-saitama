<template>
  <div class="About">
    ...
    <TextCard :title="$t('データについて')">
      {{
        $t(
          '当サイトのデータは、埼玉県のオープンデータポータルサイトからデータを取得することで情報を更新しています。'
        )
      }}<br />
      {{ $t('現在利用しているデータは以下の通りです。') }}<br /><br />
      <ul>
        <li>
          <a
        ...
        <ul>
          ...
          <li>
            {{ $t('陽性患者数（日別） (patients_summary)') }}
          </li>
        </ul>
      </ul>
      <br />{{ $t('行政の皆さまのご協力に感謝します。') }}
      <br />
      <a
      ...
      >
        ...
        {{
          $t(
            'このサイトのすべてのデータは「クリエイティブ・コモンズ 表示 2.1 日本」にて提供されています。'
          )
        }}
      </a>
    </TextCard>
